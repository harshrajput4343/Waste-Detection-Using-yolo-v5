...
def check_comet_weights(opt):
    ...
    if isinstance(opt.weights, str) and opt.weights.startswith(COMET_PREFIX):
        try:
            ...
            logger.warning("COMET WARNING: Unable to access Comet API to download weights. Proceeding without Comet.")
            logger.debug(e)
            return None
...
def check_comet_resume(opt):
    ...
    if isinstance(opt.resume, str) and opt.resume.startswith(COMET_PREFIX):
        try:
            ...
            logger.warning("COMET WARNING: Unable to access Comet API to resume run. Proceeding without Comet.")
            logger.debug(e)
            return None
